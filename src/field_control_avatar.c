...
{
    //if (FlagGet(FLAG_BADGE01_GET) == TRUE && PartyHasMonWithSurf() == TRUE && IsPlayerFacingSurfableFishableWater() == TRUE)
    //if (IsPlayerFacingSurfableFishableWater() == TRUE) //added
    //    return EventScript_UseSurf;
    if (FlagGet(FLAG_BADGE01_GET) == TRUE && (PartyHasMonWithSurf() == TRUE || CheckBagHasItem(ITEM_HM03 ,1)) && IsPlayerFacingSurfableFishableWater() == TRUE)
        return EventScript_UseSurf;
